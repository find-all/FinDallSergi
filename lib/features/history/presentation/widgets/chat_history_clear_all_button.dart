...
import 'package:fin_dall/features/history/presentation/controllers/clear_chat_history_controller.dart';
import 'package:fin_dall/features/history/presentation/widgets/history_clear_all_button.dart';
import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:loader_overlay/loader_overlay.dart';

/// A widget that displays a button to clear all chat history entries.
///
/// This widget uses Riverpod for state management and handles the process
/// of clearing all chat history, including showing loading overlays and
/// displaying success or error messages.
class ChatHistoryClearAllButton extends ConsumerWidget {
  /// Creates a [ChatHistoryClearAllButton] widget.
  const ChatHistoryClearAllButton({super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Create a paging controller for managing chat history entries
    final pagingController =
        pagingControllerProvider(fetch: fetch, pageSize: 25, initialPage: 1);

    // Listen for changes in the clear chat history state
    ref.listen<ControllerState<void>>(clearChatHistoryControllerProvider,
        (previous, next) {
      switch (next) {
    ...
    final state = ref.watch(pagingController.select((value) {
      return switch (value) {
        PageError<ChatHistoryEntry>() => true,
        PageLoadedData<ChatHistoryEntry>() => true,
        LastPage<ChatHistoryEntry>() => true,
        _ => false,
      };
    }));
    ...
    return HistoryClearAllButton(
      onPressed: state
          ? () {
              // Trigger the clear chat history action
              ref
